...
# 初始化变量
page_number = 1
max_pages = 1  # 设置要爬取的页面数，爬的页数太多会被反爬机制阻止，建议设置sleep
product_num = 0
max_product_num = 5
data = []
...
while page_number <= max_pages:
    ...
    for div in div_list:
        ...
        name = html_parse_new.find("li", string=re.compile("商品名称")).get_text().replace('商品名称：', '').strip()
        ...
        time.sleep(1)
        # 获取好评率
        good_commit_percent = driver.find_element_by_css_selector('#comment > div.mc > div.comment-info.J-comment-info > div.comment-percent > div').text
        # 获取评论关键字
        commit_tags = driver.find_elements_by_css_selector('#comment > div.mc > div.comment-info.J-comment-info > div.percent-info > div > span')
        tags = []
